...
export default function CheckoutForm({ product, productId }: CheckoutFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-lg p-8 shadow-sm">
      <h2 className="text-xl font-bold text-brand-dark mb-8 uppercase">
        Thông tin nhận sản phẩm
      </h2>

      {/* Name Field */}
      <div className="mb-8">
        <label htmlFor="name" className="block text-sm font-semibold text-brand-dark mb-3">
          Họ và tên <span className="text-brand-coral">*</span>
        </label>
        <input
          type="text"
          id="name"
      ...
      {/* Email Field */}
      <div className="mb-8">
        <label htmlFor="email" className="block text-sm font-semibold text-brand-dark mb-3">
          Email Gmail cá nhân <span className="text-brand-coral">*</span>
        </label>
        <div className="flex gap-2">
          <input
            ...
            required
            className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-gold"
          />
          <span className="px-3 py-3 bg-brand-coral text-white rounded-lg font-semibold">
            @gmail.com
          </span>
        </div>
        <p className="text-sm text-brand-coral mt-3 flex gap-2">
          <span>⚠️</span>
          <span>
      ...
      <div className="mt-12 pt-8 border-t border-gray-200 text-center text-sm text-gray-600">
        ...
        <p>
          Nếu có vấn đề gì, <a href="/" className="text-brand-gold hover:underline">liên hệ với chúng tôi</a>
        </p>
      </div>
    </form>
  )
