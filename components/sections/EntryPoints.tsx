export default function EntryPoints() {
  ...
  return (
    <section id="entry-points" className="py-20 md:py-32 px-4 bg-brand-cream">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {entryPoints.map((point, idx) => (
            <div key={idx} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow p-8 border-l-4 border-brand-gold">
              <div className="text-4xl mb-4">{point.icon}</div>
              <h3 className="text-lg font-semibold text-gray-600 mb-2">{point.title}</h3>
              <h4 className="text-2xl font-serif text-brand-dark mb-4">{point.product}</h4>
